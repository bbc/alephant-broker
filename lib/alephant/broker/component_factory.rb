  ...
  module Broker
    class ComponentFactory
      ...
      def create(id, batch_id, options)
        ...
        logger.error "Broker.ComponentFactory.create: Exception raised (ContentNotFound) for #{component_meta.component_key}"
        logger.metric "ContentNotFound"
        ErrorComponent.new(component_meta, 404, e)
      rescue => e
        logger.error "Broker.ComponentFactory.create: Exception raised (#{e.message}, #{e.backtrace.join('\n')})"
